import{bw as A,b7 as H}from"./index-D0lGjoNi.js";const _=`/*--------------------------------*- Choupo -*-----------------------*\\
...
vaporPressure
{
    ...
    coefficients  (3.74141   304.227   -5.83);
    Trange        (83  94);
}
...
vaporPressure
{
    ...
    // Poling 5e App. A (via the MIT-licensed \`chemicals\` 1.5.2 data
    // distribution; INTERIM pending primary review (Vitor))
    coefficients  (4.06280   861.340   -26.82);
    Trange        (176.31  255.79);   // clamped to the source fit range (review 2026-08-24)
}
...
vaporPressure
{
    model         Antoine;
    // log10(Psat [bar]) = A - B / (T[K] + C)
    // Landolt-Boernstein Antoine set, converted from ln/Pa to log10/bar (via
    // the MIT-licensed \`chemicals\` 1.5.2 data distribution; INTERIM pending
    // primary review (Vitor))
    coefficients  (5.09407   1886.760   -52.55);
    Trange        (330  446);
}
...
Pc          89.63;              // bar
omega       0.090;              // [-]
Tb          212.85;             // K  HEOS reference EoS; two independent
                                //    Psat curves (Poling, Perry) agree the
                                //    old 213.60 sat above 1 atm (2026-08-23)             // K
HvapTb      18670;              // J/mol
...
vaporPressure
{
    ...
    // log10(Psat [bar]) = A - B / (T[K] + C)
    // Poling 5e App. A (via the MIT-licensed \`chemicals\` 1.5.2 data
    // distribution; INTERIM pending primary review (Vitor)); consistent with
    // Tb 212.85 K (HEOS), see Tb note
    coefficients  (4.22882   806.933   -21.76);
    Trange        (186  227);
}
...
vaporPressure
{
    model         Antoine;
    // log10(Psat [bar]) = A - B / (T[K] + C)
    // CURATION FIT (2026-08-23): least-squares Antoine to the Wagner-ORIGINAL (3-6 form) curve of McGarry 1983
    // of McGarry 1983 over 230-300 K, max dev 0.05% (via the MIT-licensed
    // \`chemicals\` 1.5.2 data distribution; INTERIM pending primary review
    // (Vitor))
    coefficients  (4.46075   1028.741   -23.0);
    Trange        (230  300);
}
...
vaporPressure
{
    model         Antoine;
    // log10(Psat [bar]) = A - B / (T[K] + C)
    // CURATION FIT (2026-08-23): least-squares Antoine to the Perry 8e
    // 5-constant form over 260-330 K, max dev 0.05% (via the MIT-licensed
    // \`chemicals\` 1.5.2 data distribution; INTERIM pending primary review
    // (Vitor))
    coefficients  (4.65748   1330.447   -12.8);
    Trange        (260  330);
}
...
vaporPressure
{
    model         Antoine;
    // log10(Psat [bar]) = A - B / (T[K] + C)
    // Poling, Prausnitz & O'Connell 5e, App. A Antoine set (via the
    // MIT-licensed \`chemicals\` 1.5.2 data distribution; INTERIM pending
    // primary review (Vitor))
    coefficients  (4.29490   745.780   -14.27);
    Trange        (160  201);
}
...
vaporPressure
{
    model         Antoine;
    // log10(Psat [bar]) = A - B / (T[K] + C)
    // CURATION FIT (2026-08-23): least-squares Antoine to the Perry 8e form
    // over 2.2-5.1 K, max dev 2.4% (quantum fluid; via the MIT-licensed
    // \`chemicals\` 1.5.2 data distribution; INTERIM pending primary review
    // (Vitor))
    coefficients  (2.28233   12.765   1.4);
    Trange        (2.2  5.1);
}
...
vaporPressure
{
    ...
    // Poling 5e App. A (via the MIT-licensed \`chemicals\` 1.5.2 data
    // distribution; INTERIM pending primary review (Vitor))
    coefficients  (4.12884   654.260   -25.99);
    Trange        (181  196.91);      // upper clamped to the source fit range (review 2026-08-24)
}
...
vaporPressure
{
    ...
    // Poling 5e App. A (via the MIT-licensed \`chemicals\` 1.5.2 data
    // distribution; INTERIM pending primary review (Vitor))
    coefficients  (5.86790   682.939   -4.88);
    Trange        (107  127.56);      // upper clamped to the source fit range (review 2026-08-24)
}
...
vaporPressure
{
    ...
    // log10(Psat [bar]) = A - B / (T[K] + C)
    // Poling 5e App. A (via the MIT-licensed \`chemicals\` 1.5.2 data
    // distribution; INTERIM pending primary review (Vitor)). The old set's A
    // and B matched this one with A converted to bar, but C was
    // mistranscribed (-53.118 for -60.680) -- the 7.6 K self-contradiction
    // the 2026-08-22 audit pinned
    coefficients  (4.13361   1195.130   -60.68);
    Trange        (266  372.51);      // upper clamped to the source fit range (review 2026-08-24)
}
...
vaporPressure
{
    ...
    //  CORRECTED 2026-08-24 against the primary in hand (Poling 5e,
    //  Appendix A, section D, methyl ethanoate, CAS 79-20-9): the
    //  coefficients are the book's exactly, but the declared window read
    //  (260 380) where the FIT runs 249.90-351.11 K.  The upper end was
    //  29 K beyond the correlation -- the direction that SILENCES an
    //  extrapolation announcement the reader is owed.  Now the source's
    //  own range; cases above 351.11 K announce, as they should.
    Trange        (249.9  351.11);
}
...
vaporPressure
{
    ...
    // Poling 5e App. A (via the MIT-licensed \`chemicals\` 1.5.2 data
    // distribution; INTERIM pending primary review (Vitor))
    coefficients  (3.95606   789.624   -25.57);
    Trange        (165.2  241.61);    // clamped to the source fit range (review 2026-08-24)
}
...
`,rt=[],st=Object.assign({"../../../data/standards/components/1Butene.dat":_,"../../../data/standards/components/Ar.dat":E,"../../../data/standards/components/C.dat":R,"../../../data/standards/components/C2H.dat":N,"../../../data/standards/components/C2H3.dat":K,"../../../data/standards/components/C2H5.dat":k,"../../../data/standards/components/C2O.dat":M,"../../../data/standards/components/C3H3.dat":w,"../../../data/standards/components/C3H5.dat":q,"../../../data/standards/components/C3H7.dat":I,"../../../data/standards/components/C4H2.dat":J,"../../../data/standards/components/C4H4.dat":D,"../../../data/standards/components/C5H5.dat":V,"../../../data/standards/components/C6H5.dat":G,"../../../data/standards/components/CH.dat":F,"../../../data/standards/components/CH2.dat":x,"../../../data/standards/components/CH2CO.dat":B,"../../../data/standards/components/CH2OH.dat":L,"../../../data/standards/components/CH2_S.dat":W,"../../../data/standards/components/CH3.dat":U,"../../../data/standards/components/CH3CO.dat":j,"../../../data/standards/components/CH3O.dat":z,"../../../data/standards/components/CH3O2.dat":Q,"../../../data/standards/components/CH4.dat":Y,"../../../data/standards/components/CN.dat":Z,"../../../data/standards/components/CO.dat":X,"../../../data/standards/components/CO2.dat":$,"../../../data/standards/components/CS.dat":nn,"../../../data/standards/components/CS2.dat":en,"../../../data/standards/components/CaCO3.dat":on,"../../../data/standards/components/CaCl2.dat":an,"../../../data/standards/components/CaO.dat":tn,"../../../data/standards/components/CaSO4.dat":rn,"../../../data/standards/components/Cl.dat":sn,"../../../data/standards/components/Cl2.dat":ln,"../../../data/standards/components/ClO.dat":dn,"../../../data/standards/components/D2.dat":cn,"../../../data/standards/components/Dichloroethane.dat":pn,"../../../data/standards/components/H.dat":mn,"../../../data/standards/components/H2.dat":un,"../../../data/standards/components/H2O2.dat":hn,"../../../data/standards/components/H2S.dat":Cn,"../../../data/standards/components/H2SO4.dat":fn,"../../../data/standards/components/H3PO4.dat":gn,"../../../data/standards/components/HCCO.dat":bn,"../../../data/standards/components/HCHO.dat":Tn,"../../../data/standards/components/HCN.dat":yn,"../../../data/standards/components/HCO.dat":Pn,"../../../data/standards/components/HCl.dat":vn,"../../../data/standards/components/HNO.dat":Sn,"../../../data/standards/components/HNO3.dat":On,"../../../data/standards/components/HO2.dat":An,"../../../data/standards/components/HOCl.dat":Hn,"../../../data/standards/components/HONO.dat":_n,"../../../data/standards/components/HSO.dat":En,"../../../data/standards/components/HSO3.dat":Rn,"../../../data/standards/components/He.dat":Nn,"../../../data/standards/components/K2SO4.dat":Kn,"../../../data/standards/components/KCl.dat":kn,"../../../data/standards/components/KOH.dat":Mn,"../../../data/standards/components/LiCl.dat":wn,"../../../data/standards/components/MD3M.dat":qn,"../../../data/standards/components/MD4M.dat":In,"../../../data/standards/components/MDM.dat":Jn,"../../../data/standards/components/MgSO4.dat":Dn,"../../../data/standards/components/N.dat":Vn,"../../../data/standards/components/N2.dat":Gn,"../../../data/standards/components/N2H2.dat":Fn,"../../../data/standards/components/N2H3.dat":xn,"../../../data/standards/components/N2H4.dat":Bn,"../../../data/standards/components/N2O.dat":Ln,"../../../data/standards/components/NCO.dat":Wn,"../../../data/standards/components/NH.dat":Un,"../../../data/standards/components/NH2.dat":jn,"../../../data/standards/components/NH3.dat":zn,"../../../data/standards/components/NH4Cl.dat":Qn,"../../../data/standards/components/NNH.dat":Yn,"../../../data/standards/components/NO.dat":Zn,"../../../data/standards/components/NO2.dat":Xn,"../../../data/standards/components/NO3.dat":$n,"../../../data/standards/components/Na2CO3.dat":ne,"../../../data/standards/components/Na2SO4.dat":ee,"../../../data/standards/components/NaCl.dat":oe,"../../../data/standards/components/NaHCO3.dat":ae,"../../../data/standards/components/NaOH.dat":te,"../../../data/standards/components/O.dat":ie,"../../../data/standards/components/O2.dat":re,"../../../data/standards/components/O3.dat":se,"../../../data/standards/components/OH.dat":le,"../../../data/standards/components/OrthoDeuterium.dat":de,"../../../data/standards/components/OrthoHydrogen.dat":ce,"../../../data/standards/components/PET.dat":pe,"../../../data/standards/components/PMMA.dat":me,"../../../data/standards/components/PVC.dat":ue,"../../../data/standards/components/ParaDeuterium.dat":he,"../../../data/standards/components/ParaHydrogen.dat":Ce,"../../../data/standards/components/R11.dat":fe,"../../../data/standards/components/R113.dat":ge,"../../../data/standards/components/R12.dat":be,"../../../data/standards/components/R1234yf.dat":Te,"../../../data/standards/components/R1234zeE.dat":ye,"../../../data/standards/components/R1234zeZ.dat":Pe,"../../../data/standards/components/R134a.dat":ve,"../../../data/standards/components/R13I1.dat":Se,"../../../data/standards/components/R143a.dat":Oe,"../../../data/standards/components/R21.dat":Ae,"../../../data/standards/components/R218.dat":He,"../../../data/standards/components/R22.dat":_e,"../../../data/standards/components/R236EA.dat":Ee,"../../../data/standards/components/R245fa.dat":Re,"../../../data/standards/components/R32.dat":Ne,"../../../data/standards/components/R365MFC.dat":Ke,"../../../data/standards/components/RC318.dat":ke,"../../../data/standards/components/S.dat":Me,"../../../data/standards/components/S2.dat":we,"../../../data/standards/components/SF6.dat":qe,"../../../data/standards/components/SH.dat":Ie,"../../../data/standards/components/SO.dat":Je,"../../../data/standards/components/SO2.dat":De,"../../../data/standards/components/SO3.dat":Ve,"../../../data/standards/components/Xe.dat":Ge,"../../../data/standards/components/acetaldehyde.dat":Fe,"../../../data/standards/components/aceticAcid.dat":xe,"../../../data/standards/components/acetone.dat":Be,"../../../data/standards/components/acetylene.dat":Le,"../../../data/standards/components/akermanite.dat":We,"../../../data/standards/components/anthophyllite.dat":Ue,"../../../data/standards/components/artinite.dat":je,"../../../data/standards/components/barite.dat":ze,"../../../data/standards/components/benzene.dat":Qe,"../../../data/standards/components/bischofite.dat":Ye,"../../../data/standards/components/bloedite.dat":Ze,"../../../data/standards/components/burkeite.dat":Xe,"../../../data/standards/components/calciumHydroxide.dat":$e,"../../../data/standards/components/calciumTartrate.dat":no,"../../../data/standards/components/carbonylSulfide.dat":eo,"../../../data/standards/components/carnallite.dat":oo,"../../../data/standards/components/celestite.dat":ao,"../../../data/standards/components/chrysotile.dat":to,"../../../data/standards/components/cis2Butene.dat":io,"../../../data/standards/components/compA.dat":ro,"../../../data/standards/components/compB.dat":so,"../../../data/standards/components/compC.dat":lo,"../../../data/standards/components/cyclohexane.dat":co,"../../../data/standards/components/cyclopentane.dat":po,"../../../data/standards/components/cyclopropane.dat":mo,"../../../data/standards/components/diethylEther.dat":uo,"../../../data/standards/components/dimethylCarbonate.dat":ho,"../../../data/standards/components/dimethylEther.dat":Co,"../../../data/standards/components/diopside.dat":fo,"../../../data/standards/components/dolomite.dat":go,"../../../data/standards/components/dowthermA.dat":bo,"../../../data/standards/components/enstatite.dat":To,"../../../data/standards/components/epsomite.dat":yo,"../../../data/standards/components/ethane.dat":Po,"../../../data/standards/components/ethanol.dat":vo,"../../../data/standards/components/ethylAcetate.dat":So,"../../../data/standards/components/ethylBenzene.dat":Oo,"../../../data/standards/components/ethylene.dat":Ao,"../../../data/standards/components/ethyleneOxide.dat":Ho,"../../../data/standards/components/fluorine.dat":_o,"../../../data/standards/components/fluorite.dat":Eo,"../../../data/standards/components/forsterite.dat":Ro,"../../../data/standards/components/gaylussite.dat":No,"../../../data/standards/components/glaserite.dat":Ko,"../../../data/standards/components/glauberite.dat":ko,"../../../data/standards/components/glucose.dat":Mo,"../../../data/standards/components/goergeyite.dat":wo,"../../../data/standards/components/gypsum.dat":qo,"../../../data/standards/components/heavyWater.dat":Io,"../../../data/standards/components/hexahydrite.dat":Jo,"../../../data/standards/components/hexamethyldisiloxane.dat":Do,"../../../data/standards/components/hfe143m.dat":Vo,"../../../data/standards/components/hitecSalt.dat":Go,"../../../data/standards/components/huntite.dat":Fo,"../../../data/standards/components/hydroxyapatite.dat":xo,"../../../data/standards/components/isoButane.dat":Bo,"../../../data/standards/components/isoButene.dat":Lo,"../../../data/standards/components/isohexane.dat":Wo,"../../../data/standards/components/isopentane.dat":Uo,"../../../data/standards/components/kainite.dat":jo,"../../../data/standards/components/kalicinite.dat":zo,"../../../data/standards/components/kieserite.dat":Qo,"../../../data/standards/components/krypton.dat":Yo,"../../../data/standards/components/labile_S.dat":Zo,"../../../data/standards/components/leonhardite.dat":Xo,"../../../data/standards/components/leonite.dat":$o,"../../../data/standards/components/lithiumChlorideH2O.dat":na,"../../../data/standards/components/mXylene.dat":ea,"../../../data/standards/components/magnesite.dat":oa,"../../../data/standards/components/magnesiumHydroxide.dat":aa,"../../../data/standards/components/melanterite.dat":ta,"../../../data/standards/components/methanol.dat":ia,"../../../data/standards/components/methylAcetate.dat":ra,"../../../data/standards/components/methylLinoleate.dat":sa,"../../../data/standards/components/methylLinolenate.dat":la,"../../../data/standards/components/methylOleate.dat":da,"../../../data/standards/components/methylPalmitate.dat":ca,"../../../data/standards/components/methylStearate.dat":pa,"../../../data/standards/components/mirabilite.dat":ma,"../../../data/standards/components/misenite.dat":ua,"../../../data/standards/components/nButane.dat":ha,"../../../data/standards/components/nButanol.dat":Ca,"../../../data/standards/components/nDecane.dat":fa,"../../../data/standards/components/nDodecane.dat":ga,"../../../data/standards/components/nHeptane.dat":ba,"../../../data/standards/components/nHexane.dat":Ta,"../../../data/standards/components/nNonane.dat":ya,"../../../data/standards/components/nOctane.dat":Pa,"../../../data/standards/components/nPentane.dat":va,"../../../data/standards/components/nUndecane.dat":Sa,"../../../data/standards/components/natron.dat":Oa,"../../../data/standards/components/neon.dat":Aa,"../../../data/standards/components/neopentane.dat":Ha,"../../../data/standards/components/nesquehonite.dat":_a,"../../../data/standards/components/novec649.dat":Ea,"../../../data/standards/components/oXylene.dat":Ra,"../../../data/standards/components/pXylene.dat":Na,"../../../data/standards/components/pentahydrite.dat":Ka,"../../../data/standards/components/pirssonite.dat":ka,"../../../data/standards/components/polyethylene.dat":Ma,"../../../data/standards/components/polyhalite.dat":wa,"../../../data/standards/components/polypropylene.dat":qa,"../../../data/standards/components/polystyrene.dat":Ia,"../../../data/standards/components/potassiumBitartrate.dat":Ja,"../../../data/standards/components/propane.dat":Da,"../../../data/standards/components/propylene.dat":Va,"../../../data/standards/components/propyleneGlycol30.dat":Ga,"../../../data/standards/components/propyne.dat":Fa,"../../../data/standards/components/pyrochroite.dat":xa,"../../../data/standards/components/rhodochrosite.dat":Ba,"../../../data/standards/components/schoenite.dat":La,"../../../data/standards/components/sepiolite.dat":Wa,"../../../data/standards/components/sepiolite_d.dat":Ua,"../../../data/standards/components/siderite.dat":ja,"../../../data/standards/components/silica.dat":za,"../../../data/standards/components/strontianite.dat":Qa,"../../../data/standards/components/sucrose.dat":Ya,"../../../data/standards/components/syngenite.dat":Za,"../../../data/standards/components/talc.dat":Xa,"../../../data/standards/components/tartaricAcid.dat":$a,"../../../data/standards/components/toluene.dat":nt,"../../../data/standards/components/trans2Butene.dat":et,"../../../data/standards/components/trona.dat":ot,"../../../data/standards/components/vivianite.dat":at,"../../../data/standards/components/water.dat":tt,"../../../data/standards/components/witherite.dat":it}),h=new Map;function g(e){const o=/^\s*name\s+([^;\s]+)\s*;/m.exec(e);o&&!h.has(o[1])&&h.set(o[1],e)}function pt(e){return h.get(e)??null}function C(e,o="standard"){let n;try{n=A(H(e))}catch{return null}const a=typeof n.name=="string"?n.name:"";if(!a)return null;const i=typeof n.formula=="string"?n.formula:"",t=n.role==="nonvolatile",r=typeof n.Tc=="number"&&n.Tc>0,l=n.vaporPressure!==void 0&&n.vaporPressure!==null&&r&&!t,c=t?"nonvolatile":l?"volatile":"fragment",T=typeof n.standardThermochemistry=="object"&&n.standardThermochemistry!==null&&n.standardThermochemistry.phase==="solid",y=typeof n.dissociation=="number"?n.dissociation:1,P=n.electrolyte!==void 0&&n.electrolyte!==null||y>1&&!T,v=n.noncondensable==="true"||n.noncondensable===!0,f=n.provenance,S=!!f&&f.source==="synthetic",p=n.groups,O=!!p&&p.unifac!==void 0&&p.unifac!==null,m=u=>typeof u=="number"&&u>0?u:void 0;return{name:a,formula:i,kind:c,vleAble:l,isElectrolyte:P,isPermanentGas:v,isSynthetic:S,hasUnifac:O,origin:o,tc:m(n.Tc),pc:m(n.Pc),tb:m(n.Tb)}}const d=Object.values(st).map(e=>(g(e),C(e))).filter(e=>e!==null).sort((e,o)=>e.name.localeCompare(o.name)),mt=rt.map(e=>(g(e),C(e,"dataLocal"))).filter(e=>e!==null).sort((e,o)=>e.name.localeCompare(o.name)),lt=/(?:^|\/)constant\/components\/([^/]+)\.dat$/;function b(e){const o=e.match(lt);if(!o)return null;const n=o[1];return n.includes(".estimate-")?null:n}function dt(e){if(!e)return[];const o=new Set(d.map(a=>a.name)),n=new Map;for(const[a,i]of Object.entries(e)){const t=b(a);if(!t)continue;const r=C(i,o.has(t)?"caseShadow":"caseLocal");if(!r)continue;const s={...r,name:t},l=a.split("/").length,c=n.get(t);(!c||l<c.depth)&&n.set(t,{meta:s,depth:l})}return[...n.values()].map(a=>a.meta).sort((a,i)=>a.name.localeCompare(i.name))}function ut(e){if(!e)return{};const o=new Map;for(const[a,i]of Object.entries(e)){const t=b(a);if(!t)continue;const r=a.split("/").length,s=o.get(t);(!s||r<s.depth)&&o.set(t,{body:i,depth:r})}const n={};for(const[a,i]of o)n[`constant/components/${a}.dat`]=i.body;return n}function ht(e){const o=dt(e);if(o.length===0)return d;const n=new Map;for(const a of d)n.set(a.name,a);for(const a of o)n.set(a.name,a);return[...n.values()].sort((a,i)=>a.name.localeCompare(i.name))}function Ct(e,o=d){const n=e.trim().toLowerCase();return n?o.filter(a=>a.name.toLowerCase().includes(n)||a.formula.toLowerCase().includes(n)):o}function ft(e,o=d){return o.find(n=>n.name===e)}function gt(e){if(!e.formula)return null;const o=n=>n.toLowerCase().replace(/\s/g,"");return o(e.formula)!==o(e.name)?e.formula:null}export{d as C,mt as D,dt as a,ft as b,ut as c,gt as f,ht as m,pt as r,Ct as s};
